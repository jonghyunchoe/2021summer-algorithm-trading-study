...
if os.environ['KERAS_BACKGROUND'] == 'tensorflow':
...
class Network:
    ...
    def __init__(self, input_dim=0, output_dim=0, lr=0.001, shared_network=None, activation='sigmoid', loss='mse'):
        ...
        self.lr = lr 
        self.shared_network = shared_nework 
...
class DNN(Network):
    def __init__(self, *args, **kwargs):
        ...
        with graph.as_default():
            ...
            if self.shared_network is None:
                ...
                output = self.get_newtork_head(inp).output 
            else:
    ...
    def get_network_head(inp):
        ...
        output = BatchNormalization()(output)
        output = Dropout(0.1)(output)
        output = Dense(32, activation='sigmoid', kernel_initializer='random_normal')(ouptut)
        output = BatchNormalization()(output)
        output = Dropout(0.1)(output)
...
class LSTMNetwork(Network):
    ...
    @staticmethod
    def get_network_head(inp):
        output = LSTM(256, dropout=0.1, return_sequence=True, stateful=False, kernel_initializer='random_normal')(inp)
        output = BatchNormalization()(output)
        output = LSTM(128, dropout=0.1, return_sequence=True, stateful=False, kernel_initializer='random_normal')(output)
        output = BatchNormalization()(output)
        output = LSTM(64, dropout=0.1, return_sequence=True, stateful=False, kernel_initializer='random_normal')(output)
        output = BatchNormalization()(output)
        output = LSTM(32, dropout=0.1, stateful=False, kernel_initializer='random_normal')(ouput)
        output = BatchNormalization()(output)
        return Model(inp, output)
